...
describe('registerGlobalSchema', () => {
  it('registers global json schema', async () => {
    ...
    jest.spyOn(vscode.workspace, 'getConfiguration').mockReturnValue(stub);
    ...
    expect(stub.update.mock.calls[0][1][0]).toMatchObject({
      ...
      url: uri.toString(),
      fileMatch: expect.arrayContaining(['app.json', 'app.config.json']),
    });
  });
});
...
describe('unregisterGlobalSchema', () => {
  it('registers global json schema', async () => {
    ...
    expect(stub.update).toBeCalledWith(config.SCHEMA_PROP, [], vscode.ConfigurationTarget.Global);
  });
});
